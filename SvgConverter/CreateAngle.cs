...
namespace SvgConverter
{
    ...
    {
        public static void AddAngle(this SvgGroupElement currentGroupElement, IReadOnlyDictionary<string, string> xmlNode,
            float curLeft, float curRight, float curTop, float curBottom)
        {
            ...
            if (xmlNode.ContainsKey("Angle") && float.Parse(xmlNode["Angle"], CultureInfo.InvariantCulture) != 0.0f)
            {
                ...
                currentGroupElement.Transform = new List<SvgTransform>
                {
        ...
        public static void AddAngles(this SvgElement currentElement, IReadOnlyDictionary<string, string> xmlNode,
            float curLeft, float curRight, float curTop, float curBottom)
        {
            // Добавим угол поворота, если он есть
            if (xmlNode.ContainsKey("Angle") && float.Parse(xmlNode["Angle"], CultureInfo.InvariantCulture) != 0.0f)
